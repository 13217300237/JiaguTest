...
public class ClassLoaderHookHelper {
    ...
    public static void hook(Context context, File dexFile) {
        try {
            if (Build.VERSION.SDK_INT >= 23) {
                ClassLoaderHookHelper.hookV23(context.getClassLoader(), dexFile, context.getCacheDir());
            } else if (Build.VERSION.SDK_INT >= 19) {
                ClassLoaderHookHelper.hookV19(context.getClassLoader(), dexFile, context.getCacheDir());
            } else if (Build.VERSION.SDK_INT >= 14) {
                ClassLoaderHookHelper.hookV14(context.getClassLoader(), dexFile, context.getCacheDir());
            }
    ...
    private static void hookV23(ClassLoader classLoader, File outDexFilePath, File optimizedDirectory)
...
//        4.获得DexPathList对象中dexElements 属性的真实值
    ...
    private static void hookV19(ClassLoader classLoader, File outDexFilePath, File optimizedDirectory) throws IllegalAccessException, InvocationTargetException {
    ...
    private static void hookV14(ClassLoader classLoader, File outDexFilePath, File optimizedDirectory) throws IllegalAccessException, InvocationTargetException {
        Field pathList = ReflectionUtil.getField(classLoader, "pathList");//1、获得DexPathList pathList 属性
        Object dexPathListObj = pathList.get(classLoader); //2、获得DexPathList pathList对象
        Field dexElementsField = ReflectionUtil.getField(dexPathListObj, "dexElements");//3、获得DexPathList的dexElements属性
        Object[] oldElements = (Object[]) dexElementsField.get(dexPathListObj);//4、获得pathList对象中 dexElements 的属性值
